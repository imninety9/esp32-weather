...
class State:
    __slots__ = (
        ...
        "apm_running",
        "wifi_ready_event", "all_tasks_done_event", "wifi_tasks_remaining",
    ...
    def __init__(self):
        ...
        # WiFi coordination
        self.wifi_ready_event = asyncio.Event()
        self.all_tasks_done_event = asyncio.Event()
        self.wifi_tasks_remaining = 0
...
def signal_task_done(state):
    """Called by WiFi-dependent tasks when they finish (success or failure)."""
    state.wifi_tasks_remaining -= 1
    if state.wifi_tasks_remaining <= 0:
        state.all_tasks_done_event.set()
...
async def mqtt_publish_task(rtc, state, sd):
    """
    MQTT publish task - waits for WiFi to be ready, then publishes sensor data.
    Runs in parallel with OWM task.
    """
    ...
    while True:
        try:
            ...
            await state.wifi_ready_event.wait()
            ...
            if state.mqtt_client and state.mqtt_ok:
                ...
                mqtt_msgs[4] = _STATUS_FMT % (
                                    row[5], format_uptime(state.boot_ticks, now_ticks),
                ...
                try:
                    ...
                    state.mqtt_ok = False
                    sd.append_error("mqtt_publish_fail", str(e), ts=rtc_tup(rtc))
            
                state.heartbeat = now_ticks
            
        except Exception as e:
            sd.append_error("mqtt_publish_task_error", str(e), ts=rtc_tup(rtc))
        finally:
            # Always signal done (success or failure)
            signal_task_done(state)
            
            
async def owm_task(rtc, state, owm_client, sd):
    """
    OWM fetch task - waits for WiFi to be ready, then fetches weather data.
    Runs in parallel with MQTT task.
    """
    # fetch aqi each 60 minutes only
    aqi_count = 0
    aqi_cycle = max(1, config.OWM_AQI_FETCH_INTERVAL_SECS // config.WIFI_TASK_INTERVAL_SECS)
...
async def wifi_manager_task(rtc, state, sd):
    """
    Scheduled WiFi manager - turns WiFi on every 15 minutes.
    Waits for all WiFi-dependent tasks to complete, then turns off.
    """
    interval = config.WIFI_TASK_INTERVAL_SECS
    wifi_task_timeout = config.WIFI_TASK_TIMEOUT_SECS # Max time WiFi stays on
    wifi_expected_tasks = config.WIFI_EXPECTED_TASKS # MQTT + OWM
        ...
        try:
            ...
            if not state.wlan or not state.wlan.isconnected():
                try:
                    ...
                    # Clear any pending task signals
                    state.wifi_ready_event.clear()
                    state.all_tasks_done_event.clear()
                    
                    # Backoff before next attempt
                    backoff = compute_backoff(WIFI_BASE_BACKOFF, wifi_fail_count, WIFI_MAX_BACKOFF, BACKOFF_JITTER_PCT) # wifi_wait
                    await asyncio.sleep(backoff)
                    continue
                
            # === STEP 2: Try to Connect MQTT ===
            ...
            # === STEP 3: Signal WiFi is ready ===
            state.wifi_tasks_remaining = wifi_expected_tasks
            state.all_tasks_done_event.clear()
            state.wifi_ready_event.set()
            
            # === STEP 4: Wait for tasks to complete (with timeout) ===
            try:
                await asyncio.wait_for(state.all_tasks_done_event.wait(), timeout=wifi_task_timeout)
            except asyncio.TimeoutError:
                sd.append_error("wifi_task_timeout", f"timeout_{wifi_task_timeout}s", 
                              ts=rtc_tup(rtc), min_interval=900)
                
            # === STEP 5: Disconnect WiFi (power save) ===
            try:
                # Disconnect MQTT
            ...
            sd.append_error("wifi_manager_error", str(e), ts=rtc_tup(rtc))
        
        # Sleep until next WiFi cycle
        await asyncio.sleep(interval)
...
async def lcd_timeout_watcher(state, lcd):
    ...
    while True:
        ...
        while state.lcd_on:
            if time.ticks_diff(time.ticks_ms(), state.last_lcd_press) >= timeout_ms:
            ...
            # Only check every second while LCD is on
...
async def main():
    ...
    asyncio.create_task(mqtt_publish_task(rtc, state, sd))
    asyncio.create_task(owm_task(rtc, state, owm_client, sd))
    asyncio.create_task(wifi_manager_task(rtc, state, sd))
