...
def sensor_and_log_task(rtc, state, sensors, sd):
    """Read sensors every SENSOR_INTERVAL_SECS, build row, log to SD."""
        try:
            gc.collect()

            # 1. Read all local sensors
            ...
            # 3. Update row in state (uses last known APM data)
            update_row(state.sensor_row, ts, sensor_data, state.apm)
...
def apm10_task(rtc, state, apm_sensor, sd):
    """
    ...
    if apm_sensor is None:
        ...
        try:
            ...
            time.sleep(STABILIZATION_SECS) # warm-up for stabilization
            ...
            for i in range(3):
                ...
                if data and data[0] is not None:
                    ...
                    if i == 0:
                        ...
                        total_pm10 += data[2]
                        n += 1
                time.sleep(1.2) # APM10 min 1s interval
