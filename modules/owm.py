# owm.py -- fetch OpenWeatherMap current weather (simple, cached)

import urequests

class OWMClient:
    __slots__ = ("url", "url_aqi", "headers", "_cache_weather", "_cache_aqi")
    
    def __init__(self, url, url_aqi, headers=None):
        self.url = url
        self.url_aqi = url_aqi
        self.headers = headers
        self._cache_weather = [None]*4 # temperature, feels_like_temp, humidity, pressure
        self._cache_aqi = [None]*3 # pm2_5, pm10, no2

    def fetch_weather(self):
        try:
            response = urequests.get(self.url, timeout=5)
            data = response.json()
            response.close()

            main = data.get('main')
            if main:
                self._cache_weather[0] = main.get('temp') # tempearture
                self._cache_weather[1] = main.get('feels_like') # feels_like_temp
                self._cache_weather[2] = main.get('humidity') # humidity
                self._cache_weather[3] = main.get('grnd_level') * 100 # pressure - Convert hPa to Pa [ground level pressure]
            del data
        except Exception:
            raise
        
        return self._cache_weather

    def fetch_aqi(self):
        try:
            response = urequests.get(self.url_aqi, timeout=5)
            data = response.json()
            response.close()

            lst = data.get('list')
            if lst:
                comp = lst[0].get('components')
                self._cache_aqi[0] = comp.get('pm2_5') # conc: μg/m3
                self._cache_aqi[1] = comp.get('pm10') # conc: μg/m3
                self._cache_aqi[2] = comp.get('no2') # conc: μg/m3
            del data
        except Exception:
            raise
        
        return self._cache_aqi
    